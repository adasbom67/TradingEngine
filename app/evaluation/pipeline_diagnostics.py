...
class PipelineDiagnostics:
    ...
    def first_zero_stage(self) -> str | None:
        stages = [
            ...
            ("candidates_evaluated", self.candidates_evaluated), ("candidates_ranked", self.candidates_ranked),
        ]
        positive_indexes = [i for i, (_, v) in enumerate(stages) if v > 0]
        if not positive_indexes:
            return stages[0][0]
        for name, value in stages[min(positive_indexes):]:
            if value == 0:
                return name
        return None
